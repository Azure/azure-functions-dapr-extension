...
namespace Microsoft.Azure.Functions.Extensions.Dapr.Core
{
    ...
    public class DaprBindingMessage
    {
        ...
        /// </summary>
        /// <param name="value">The data of the message sent to the Dapr binding.</param>
        ...
        /// <exception cref="ArgumentException">Thrown if <paramref name="value"/> is not serializable to JSON.</exception>
        public DaprBindingMessage(object value, Dictionary<string, object>? metadata = null, string? binding = null, string? operation = null)
        {
            if (value == null)
            {
            ...
            try
            {
                serializedData = JsonSerializer.Serialize(value, JsonUtils.DefaultSerializerOptions);
            }
            catch (Exception e)
            {
                throw new ArgumentException("The data object must be serializable to JSON.", nameof(value), e);
            }
