...
> For reference or to skip ahead, you can clone and navigate to a working version of [this quickstart app here](../quickstarts/javascript/).
...
1. Confirm your `function.json` matches the completed [sample here](../quickstarts/javascript/HttpTrigger/function.json).
...
1. Specify the port you want Dapr to listen on in the `local.settings.json` file by adding a value for `DAPR_HTTP_PORT` of `3501` as shown in [the quickstart sample](../quickstarts/javascript/local.settings.json).  If you wanted to debug multiple apps at the same time you would need to assign unique ports for each.
...
1. Replace the `timerTrigger` trigger with a Dapr topic trigger:
    ...
    ```
    You can see a completed `function.json` in the [quickstart sample here](../quickstarts/javascript/DaprSubscribeTrigger/function.json).
...
1. Copy the contents of [this deployment file](../quickstarts/javascript/deploy.yaml) to your machine in a new file named `deploy.yaml`.
