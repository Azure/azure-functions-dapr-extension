...
1. Open a terminal and run the command to test this function using the `dapr` CLI:
    `dapr run --app-id function-app --port 3501 -- func start -p 7071`

1. Make an HTTP POST to the local function.  You can use a tool like cUrl, [Postman](), or [REST Client]().
    ```bash
...
1. Test out triggering the function with an HTTP POST to `https://{EXTERNAL IP}/api/state/hello`
